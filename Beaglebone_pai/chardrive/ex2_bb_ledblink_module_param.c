/**
 * @file    ex1_bb_ledblink_module_param.c
 * @author  Santosh Pai
 * @date    18 Oct 2015
 * @version 0.1
 * @brief  An introductory "led blinking" loadable kernel module (LKM) that can display a message
 * in the /var/log/kern.log file when the module is loaded and removed. The module can accept an
 * argument when it is loaded -- the led blink count, which appears in the kernel log files.
*/
...
MODULE_DESCRIPTION("SIMPLE LED BLINKING DRIVER BY Taking Blink count as a module Parameter");//< The description -- see modinfo
...
int loop = 1;	// < This is used passed the blink count from LKM
module_param(loop,int, 0);//< This macro is used to pass the variable
 ...
 */
static int __init ex2_bb_ledblink_module_param_init( void ){
	printk(KERN_INFO "--Ex2 LED: Initialization of LED_BLINKING Program %s--\n",__FUNCTION__);
	...
	// run the led for loop count times
	loop++;
	while(loop){
 ...
 */
static void __exit ex2_bb_ledblink_module_param_exit( void ){

	gpio_set_value(gpioLED49, 0);	//< Turn the LED off, indicates device was unloaded
	gpio_free(gpioLED49);         // Free the LED GPIO
	printk(KERN_INFO "Ex2 LED: Goodbye from %s BBB LED LKM!\n",__FUNCTION__);

}

module_init(ex2_bb_ledblink_module_param_init);
module_exit(ex2_bb_ledblink_module_param_exit);
